use ilp::{IlpPacket, IlpPrepare, IlpFulfill, IlpReject};
use std::io::Cursor;
...
static ref PEER_PROTOCOL_FULFILLMENT: Bytes = Bytes::from(vec![0; 32]);
  static ref PEER_PROTOCOL_CONDITION: Bytes = Bytes::from(vec![102,104,122,173,248,98,189,119,108,143,193,139,142,159,142,32,8,151,20,133,110,226,51,179,144,42,89,29,13,95,41,37]);
}
...
pub fn get_config(
  plugin: impl Plugin,
) -> impl Future<Item = (IldcpResponse, impl Plugin), Error = ((), impl Plugin)> {
  ...
  plugin
    ...
    .map_err(move |err| {
      ...
      plugin.into_future().and_then(|(next, plugin)| {

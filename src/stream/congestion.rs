...
impl CongestionController {
  ...
  pub fn prepare(&mut self, id: u32, amount: u64) {
    ...
    debug!("Prepare packet of {}, amount in flight is now: {}", amount, self.amount_in_flight);
  }
  ...
  pub fn fulfill(&mut self, id: u32) {
    if let Some(amount) = self.packets.remove(&id) {
      ...
      if self.state == CongestionState::SlowStart {
        ...
        debug!("Fulfilled packet of {}, doubling max in flight to: {}", amount, self.max_in_flight);
      } else {
        self.max_in_flight += self.increase_amount;
        debug!("Fulfilled packet of {}, increasing max in flight to: {}", amount, self.max_in_flight);
      }
    }
  }
  ...
  pub fn reject(&mut self, id: u32, error_code: &str) {
    if let Some(amount) = self.packets.remove(&id) {
      ...
      if error_code == "T04" {
        ...
        self.max_in_flight = max((self.max_in_flight as f64 / self.decrease_factor).floor() as u64, 1);

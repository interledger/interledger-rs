...
curl \
-H "Authorization: Bearer hi_charlie" \
http://localhost:9770/accounts/1/balance
...
<!--! printf "Sending payment of 80 from Alice to Charlie through Bob\n" -->

```bash
# Sending payment of 80 from Alice to Charlie through Bob
curl \
    -H "Authorization: Bearer in_alice" \
    -H "Content-Type: application/json" \
    -d '{"receiver":"http://localhost:9770","source_amount":80}' \
...
### Check the Settlement Transaction Generation on XRPL
You'll find transaction logs in your settlement engine logs. Try:

```bash #
cat logs/node-charlie-settlement-engine-xrpl.log | grep "Got incoming payment from .*:  {" | awk '{sub("^Got incoming payment from[^{]*", "", $0); print}'
```

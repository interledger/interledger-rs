...
impl ConnectionGenerator {
    ...
    ///
    /// If the feature flag "fulfill-packets-for-js-receiver" is enabled, it will use the exact same algorithm
    /// to generate the shared_secret from the server_secret as is used by the Javascript
    /// ilp-protocol-stream library. This enables this receiver to fulfill packets created with
    /// the STREAM details generated by a JS STREAM server.
    pub fn rederive_secret(&self, destination_account: &Address) -> Result<[u8; 32], ()> {
        ...
        if let Ok(local_part) = base64::decode_config(local_part, base64::URL_SAFE_NO_PAD) {
        ...
        #[cfg(feature = "fulfill-packets-for-js-receiver")]
        {
            // The JS receiver computes the HMAC with the token _encoded as UTF8_,
            // whereas this implementation first decodes the token from base64
            let shared_secret = hmac_sha256(&self.secret_generator[..], &local_part.as_bytes()[..]);
            return Ok(shared_secret);
        }
        #[cfg(not(feature = "fulfill-packets-for-js-receiver"))]
        Err(())
    }
}
...
mod receiving_money {
    ...
    fn fulfills_packets_sent_to_javascript_receiver() {
        ...
        let fulfill = receive_money(&shared_secret, &ilp_address, prepare)

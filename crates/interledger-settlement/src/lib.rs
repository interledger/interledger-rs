...
pub struct SettlementMessageHandler<S, A> {
    ilp_address: Bytes,
    next: S,
    http_client: Client,
...
impl<S, A> SettlementMessageHandler<S, A>
where
    S: IncomingService<A>,
    A: SettlementAccount,
{
    pub fn new(ilp_address: Bytes, next: S) -> Self {
        SettlementMessageHandler {
            next,
            ilp_address,
...
impl<S, A> IncomingService<A> for SettlementMessageHandler<S, A>
where
    S: IncomingService<A>,
    A: SettlementAccount,
{

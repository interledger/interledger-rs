...
impl SettlementClient {
    ...
    pub fn send_settlement<A: SettlementAccount + IldcpAccount>(
        ...
        if let Some(mut settlement_engine_url) = account.settlement_engine_url() {
            if let Some(settlement_engine_scale) = account.settlement_engine_asset_scale() {
                let amount = if settlement_engine_scale >= account.asset_scale() {
                    amount * 10u64.pow(u32::from(settlement_engine_scale - account.asset_scale()))
                } else {
                    amount / 10u64.pow(u32::from(settlement_engine_scale - account.asset_scale()))
                };
